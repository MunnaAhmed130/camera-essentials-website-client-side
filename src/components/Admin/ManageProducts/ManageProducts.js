    ...
    return (
        <div>
            ...
            <Row xs={1} sm={1} md={2} lg={3} className="">
                {
                    products.map(product => <ManageProduct key={product._id} product={product} success={success} handleDeleteOrder={handleDeleteOrder}></ManageProduct>)
                }
                {success && <Alert severity="error">A Product has been Successfully Deleted</Alert>}
            </Row>
        </div>
    );
};
